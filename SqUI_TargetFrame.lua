-----------------------------------------------------------------------------------------------
-- Client Lua Script for SquishyUI
...
-----------------------------------------------------------------------------------------------
-- SquishyUI OnDocLoaded
-----------------------------------------------------------------------------------------------
function SqUI_UF:OnDocumentReady()
...
function SqUI_UF:OnFrame()
	self:OnCharacterLoaded()
end

function SqUI_UF:OnCharacterLoaded()
...
--[[
	...
	self.wndToTFrame = self.wndMainClusterFrame:FindChild("TopCluster")
...
--[[
	...
	self.nRaidMarkerLeft, self.nRaidMarkerTop, self.nRaidMarkerRight, self.nRaidMarkerBottom = self.wndMainClusterFrame:FindChild("RaidMarker"):GetAnchorOffsets()
...
function SqUI_TargetFrame:OnUpdate()
  ...
  self.wndMainClusterFrame:Show(self.wndMainClusterFrame:IsShown())
  ...
  if unitTarget ~= nil then
--[[ Don't care about Mini Cluster frames for now    
	-- Cluster info
...
--  self.wndLargeFrame:FindChild("TargetModel"):SetData(unitTarget)
  ...
  self:SetTargetForFrame(self.wndMainClusterFrame, unitTarget, bTargetChanged)
...
--[[
  ...
  -- Raid Marker
  local wndRaidMarker = self.wndMainClusterFrame:FindChild("RaidMarker")
...
--  self.wndMainClusterFrame:FindChild("TargetModel"):SetCostume(unitTarget)
end
...
function SqUI_TargetFrame:UpdateAlternateFrame(unitToT)
  ...
  local wndFrame = self.wndMainClusterFrame:FindChild("BottomCluster")
...
--[[
  ...
  if unitToT:GetType() == "Player" then
    ...
    wndFrame:FindChild("Name"):SetTextColor(crColorToUse)
  end
...
--  wndFrame:FindChild("Name"):SetText(unitToT:GetName())
--[[
  if eDisposition == Unit.CodeEnumDisposition.Friendly or unitToT:IsThePlayer() then
  ...
  -- Resize
--  wndFrame:FindChild("ShieldProgressBar"):EnableGlow(nShieldCurr > 0)
--  self:SetBarValue(wndFrame:FindChild("ShieldProgressBar"), 0, nShieldCurr, nShieldMax) -- Only the Curr Shield really progress fills
...
--[[
  ...
  -- Sprite
  local wndHealth = wndFrame:FindChild("BottomClusterProgressBar")
...
function SquishyUI:HideClusterFrames()
  for idx = 2, 5 do
    self.arClusterFrames[idx]:Show(false)
    self.arClusterFrames[idx]:SetData(nil)
  end
...
function SqUI_TargetFrame:SetTargetForFrame(wndFrame, unitTarget, bTargetChanged)
  ...
  if unitTarget then
    ...
    wndFrame:FindChild("Name"):SetText(strName)
...
--[[
    ...
    if nLevel == nil then
      wndFrame:FindChild("Level"):SetText(Apollo.GetString("CRB__2"))
      wndFrame:FindChild("Level"):SetTextColor(karConInfo[1][2])
      wndFrame:FindChild("Level"):SetTooltip("")
    else
      wndFrame:FindChild("Level"):SetText(unitTarget:GetLevel())

      if unitTarget:IsScaled() then
        wndFrame:FindChild("ScalingMark"):Show(true)
        wndFrame:FindChild("Level"):SetTextColor(kcrScalingCColor)
        ...
        wndFrame:FindChild("ScalingMark"):Show(false)
        local nCon = self:HelperCalculateConValue(unitTarget)
        wndFrame:FindChild("Level"):SetTextColor(karConInfo[nCon][2])
    ...
    if strUnitType == "Player" or strUnitType == "Pet" or strUnitType == "Esper Pet" then
      ...
      wndFrame:FindChild("GroupMark"):Show(false)
      wndFrame:FindChild("Name"):SetTextColor(crColorToUse)
    else -- NPC
      wndFrame:FindChild("GroupMark"):Show(unitTarget:GetGroupValue() > 0)
      wndFrame:FindChild("GroupMark"):SetText(unitTarget:GetGroupValue())
      ...
      wndFrame:FindChild("GroupMark"):SetTooltip(strGroupTooltip)
    end
...
--      wndFrame:FindChild("TargetModel"):SetTooltip(unitTarget == GameLib.GetPlayerUnit() and "" or strTooltipRank)
      ...
      if unitTarget:GetArchetype() and wndFrame:FindChild("ClassIcon") then
        wndFrame:FindChild("ClassIcon"):SetSprite(unitTarget:GetArchetype().strIcon)
      end
    else
...
function SqUI_TargetFrame:UpdateCastingBar(wndFrame, unitCaster)
  ...
  local wndCastFrame = wndFrame:FindChild("CastBar")
  local wndCastProgress = wndFrame:FindChild("CastingProgressBar")
  local wndCastName = wndFrame:FindChild("CastName")
  local wndCastIcon = wndFrame:FindChild("CastIcon")
...
function SqUI_TargetFrame:SetTargetHealthAndShields(wndTargetFrame, unitTarget)
  ...
  if unitTarget:GetType() == "Simple" then -- String Comparison, should replace with an enum
    self.wndMainClusterFrame:FindChild("HealthProgressBar"):SetText("")
    self.wndMainClusterFrame:FindChild("MaxShield"):Show(false)
--    self.wndMainClusterFrame:FindChild("MaxAbsorb"):Show(false)
    return
  end
...
--  self:SetBarValue(self.wndLargeFrame:FindChild("AbsorbCapacityTint"), 0, nAbsorbCurr, nAbsorbMax)
  ...
  -- Bars
  self.wndMainClusterFrame:FindChild("HealthProgressBar"):SetMax(nHealthMax);
  self.wndMainClusterFrame:FindChild("HealthProgressBar"):SetProgress(nHealthCurr);

  self.wndMainClusterFrame:FindChild("ShieldProgressBar"):SetMax(nShieldMax);
  self.wndMainClusterFrame:FindChild("ShieldProgressBar"):SetProgress(nShieldCurr);
  self.wndMainClusterFrame:FindChild("ShieldProgressBar"):SetBarColor(tColors.blue);
--[[
  ...
  self.wndMainClusterFrame:FindChild("MaxShield"):Show(nShieldCurr > 0 and nShieldMax > 0)-- and unitTarget:ShouldShowShieldCapacityBar())
  ...
  if nVisibility == 2 then -- show x/y
    wndHealth:SetText(strTextXY)
  elseif nVisibility == 3 then --show %
    wndHealth:SetText(strTextPCT)
  else --on mouseover
    wndHealth:SetText("")
  end

  wndHealth:SetTooltip(strTooltip)
end
...
local UnitFramesInstance = SqUI_UF:new()
SqUI_UF:Init()
